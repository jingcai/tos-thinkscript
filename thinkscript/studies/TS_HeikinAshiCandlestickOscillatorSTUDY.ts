# HeikinAshiCandlestickOscillator
...
plot HACO = if !isNan(close) then 1 else double.nan;
HACO.setPaintingStrategy(paintingStrategy.HISTOGRAM);
HACO.setLineWeight(5);
HACO.assignValueColor(if signal then color.green else color.red);