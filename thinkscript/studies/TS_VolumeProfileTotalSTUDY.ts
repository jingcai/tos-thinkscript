...
input barColorRed = 0;
input barColorGreen = 155;
input barColorBlue = 255;
input barWeight = 3;
...
plot histVwap = TotalSum(hlc3*volume) / TotalSum(volume);
def vwapSD = sqrt(TotalSum(Sqr(hlc3-histVwap)*volume)/TotalSum(volume));
histVwap.SetDefaultColor(color.magenta);
histVwap.SetLineWeight(3);
#histVwap.hide();

plot r1 = histVwap+VWAPStdev1*vwapSD;
plot s1 = histVwap-VWAPStdev1*vwapSD;
plot r2 = histVwap+VWAPStdev2*vwapSD;
plot s2 = histVwap-VWAPStdev2*vwapSD;
plot r3 = histVwap+VWAPStdev3*vwapSD;
plot s3 = histVwap-VWAPStdev3*vwapSD;
...
histVwap.SetStyle(curve.SHORT_DASH);
...
rec vwapExt = if IsNaN(close) then vwapExt[1] else histVwap;
rec r1Ext = if IsNaN(close) then r1Ext[1] else r1;
rec r2Ext = if IsNaN(close) then r2Ext[1] else r2;
rec r3Ext = if IsNaN(close) then r3Ext[1] else r3;
rec s1Ext = if IsNaN(close) then s1Ext[1] else s1;
rec s2Ext = if IsNaN(close) then s2Ext[1] else s2;
rec s3Ext = if IsNaN(close) then s3Ext[1] else s3;

plot vwap = if IsNaN(close) then vwapExt else double.nan;
vwap.SetDefaultColor(color.magenta);
vwap.SetLineWeight(3);
vwap.SetStyle(curve.SHORT_DASH);

plot vwapR1 = if IsNaN(close) then r1Ext else double.nan;
vwapR1.SetDefaultColor(color.orange);
vwapR1.SetLineWeight(2);
vwapR1.SetStyle(curve.SHORT_DASH);

plot vwapR2 = if IsNaN(close) then r2Ext else double.nan;
vwapR2.SetDefaultColor(color.pink);
vwapR2.SetLineWeight(2);
vwapR2.SetStyle(curve.SHORT_DASH);

plot vwapR3 = if IsNaN(close) then r3Ext else double.nan;
vwapR3.SetDefaultColor(color.cyan);
vwapR3.SetLineWeight(2);
vwapR3.SetStyle(curve.SHORT_DASH);

plot vwapS1 = if IsNaN(close) then s1Ext else double.nan;
vwapS1.SetDefaultColor(color.orange);
vwapS1.SetLineWeight(2);
vwapS1.SetStyle(curve.SHORT_DASH);

plot vwapS2 = if IsNaN(close) then s2Ext else double.nan;
vwapS2.SetDefaultColor(color.pink);
vwapS2.SetLineWeight(2);
vwapS2.SetStyle(curve.SHORT_DASH);

plot vwapS3 = if IsNaN(close) then s3Ext else double.nan;
vwapS3.SetDefaultColor(color.cyan);
vwapS3.SetLineWeight(2);
vwapS3.SetStyle(curve.SHORT_DASH);
